...
public class BBur extends Item {
...
public String getUnlocalizedName(ItemStack par1BBur)
	{
	    int i = MathHelper.clamp_int(par1BBur.getItemDamage(), 0, 1);
	    return super.getUnlocalizedName() + Meta[i];
	}

@SideOnly(Side.CLIENT)
public void getSubItems(Item par1BBur, CreativeTabs par2Tabs, List par3List)
	{
	    for (int i = 0; i < 2; ++i)
	    {
	    	par3List.add(new ItemStack(par1BBur, 1, i));
	    }
	}
...
public void registerIcons(IIconRegister par1Icons)
	{
	    ...
	    {
	        this.IIcons[i] = par1Icons.registerIcon(Ref.MOD_ID + ":" + "BBur" + Meta[i]);
	    }
	}
